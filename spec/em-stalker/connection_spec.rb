...
describe EMStalker::Connection do
  ...
  describe 'defaults' do
    ...
    it 'watch and use to provided tube on connect' do
      ...
      conn = EMStalker::Connection.new(:tube => "mytube")
      conn.connected
    end
...
if false
    ...
    it 'the "delete" command' do
      ...
      job = EMStalker::Job.new(nil, 1, "body")
      conn.delete(job)
    end
    ...
    describe 'stats' do
      ...
      it 'job' do
        job = EMStalker::Job.new(nil, 42, "blah")
      ...
      it 'throws exception on invalid param' do
        connection_mock.should_not_receive(:send)
        lambda { conn.stats(:blah) }.should raise_error(EMStalker::InvalidCommand)
      end
    end
    ...
    describe 'list' do
      ...
      it 'throws exception on invalid param' do
        connection_mock.should_not_receive(:send)
        lambda { conn.list(:blah) }.should raise_error(EMStalker::InvalidCommand)
      end
    end
    ...
    describe 'peek' do
      ...
      it 'throws exception on invalid param' do
        connection_mock.should_not_receive(:send)
        lambda { conn.peek(:blah) }.should raise_error(EMStalker::InvalidCommand)
      end
    end
  end
  ...
  describe 'reconnect' do
    ...
    it 'raises exception if it fails more then RETRY_COUNT times' do
      ...
      lambda { conn.disconnected }.should raise_error(EMStalker::Disconnected)
    end
    ...
    it 'resets the retry count on connection' do
      ...
      5.times { conn.disconnected }
      conn.connected
      lambda { conn.disconnected }.should_not raise_error(EMStalker::Disconnected)
    end
